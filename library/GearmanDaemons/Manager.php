...
class Manager {
    ...
    /**
     * Maximum time a worker will run. In Seconds
    ...
    /**
     * @param  string|array|Zend_Config|null, or options array or Zend_Config instance
     * @return void
     */
    public function __construct($spec)
    {
    	...
    	if (array_key_exists('daemon', $this->_options) && $this->_options['daemon']==true) {
    	    $this->_daemon = true;
    	    $this->_daemonize();
    	}
    	
    	if (array_key_exists('recover_workers', $this->_options) && $this->_options['recover_workers']==true) {
    ...
    protected function _signalAllWorkers($signal)
    {
       foreach ($this->_workers as $worker) {
           $this->_logger->info('Sending ' . $signal . ' Signal ' .$worker->getPid());
    ...
    protected function _spawnWorker(WorkerAbstract $worker)
    {
    	...
    	if ($pid === -1) {
    	    ...
    	    $gearmanWorker->addFunction($worker->getRegisterFunction(), [$worker, 'execute']);
    		...
    		$this->_logger->info('Worker ' . $worker->getRegisterFunction() . ' to ' . $pid);
    	    $worker->setPid($pid);
